...
class Landscape(object):
    ...
    Attributes:
        :int L:             Square dimension of the landscape lattice.
        :list land:         2D list representing lattice (is filled with trees)
        :list top:          2D list representing topography
        :dict neighbors:    Dictionary descrbing neighbors of each cell.
        :list indices:      List of all indices
        :list one_fire:     List of sites on fire.
    """
    
    def __init__(self, L=20):
        """
        Constructor for the Landscape.
        
        Parameters:
            :int L: The square dimension of the landscape lattice.
    ...
    def get_neighbors(self):
        """
        Creates an dictionary of adjacency lists for each site on the lattice:
    ...
    def initialize_fire(self, amt_spread):
        """
        Randomly construct a connected component of sites that have the value 
        of 1. Do this by selecting a coordinate on the lattice at random, then 
        following random edges of its neighbors to ignite (turn 1).
        Continue following excess edges for the input amount of spreading. 
        Nearest neighbors to the selected coordinate are always burned. This 
        way, a centroid is defined and the fire is not biased in one direction.
        
        Parameters:
            :int amt_spreading: the number of edges to follow out from the 
                                point of outbreak.
    ...
    def get_sites_on_fire(self):
        """
        Gets the indices of sites that are assigned the value 1 (on fire).
    ...
    def perlin_terrain(self, f, noise, num_targets):
        ...
        Parameters:
            :int f: a scale factor and the height displacement (phase 
                    displacement shift/ translation).

            :func noise: the noise function that defines the deformation
                         of a point across the 2D space.
                         Acceptible Input:
                        'Sinosoid': generates a sinosoidalsurface centered at 
                        f+1. (see: 
    ...
    def display_topography(self):
        """
        Graphically displays the topography of the map.
