...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 16 09:46:58 2019
...
def make_prob_fire(individual, toolbox):
    ...
    def prob_func(neighborhood):
        ...
        mean_ws = neighborhood[:, L_WS].mean()
        mean_wd = neighborhood[:, L_WD].mean()
        ...
        wind_term = np.exp(5*np.cos(mean_wd))*mean_ws

        args = (*cell, mean_temp, mean_hum, mean_weighted_fire, wind_term)
        try:
            # print("\n\n")
            # print(func)
            # print(individual)
            ...
            # print("dz_sum", dz_sum)
            return func(*cell, mean_temp, mean_hum, mean_weighted_fire, wind_term)
        except ValueError:
            # print("\n\n")
...
def main(init_landscape_path, final_landscape_path, max_time):
    ...
    # The primitives of the tree and their arities.
    pset = gp.PrimitiveSet("main", 8, prefix='x')
    pset.renameArguments(x0='fire', x1='tree', x2='z', x3='temp', x4='hum',
                         x5='mean_temp', x6='mean_hum', x7='mean_weighted_fire',x8='wind_term')
    pset.addPrimitive(operator.add, 2)
    pset.addPrimitive(operator.sub, 2)
    pset.addPrimitive(operator.mul, 2)
    pset.addPrimitive(protectedDiv, 2)
    ...
    pop = toolbox.population(n=pop_size)
    hof = tools.HallOfFame(1)
    pop, log = algorithms.eaSimple(pop, toolbox, 0.5, 0.1, 1, stats=mstats, halloffame=hof, verbose=True)
    tree = gp.PrimitiveTree(hof[0])
    print(tree)
    # print(hof)
    # print(log)
